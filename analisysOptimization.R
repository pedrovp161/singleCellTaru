...
pb21_ns <- Read10X(data.dir = "data/pb21_ns")

# Initialize the Seurat object with the raw (non-normalized data).
pb21_ns <- CreateSeuratObject(counts = pb21_ns, project = "leprosy_t_cell", min.cells = 3, min.features = 200)

pb21_ns

#Filtragem posterior
pb21_ns[["percent.mt"]] <- PercentageFeatureSet(pb21_ns, pattern = "^MT-") 
VlnPlot(pb21_ns, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)

#Normalizacao de Dados
pb21_ns$nUMI <- Matrix::colSums(GetAssayData(pb21_ns, slot = "counts"))
summary(pb21_ns$nUMI)

#Avalia qual valor de escala funcionara melhor para a normalizacao
# Step 1: Plot UMI distribution
vln_plot <- VlnPlot(pb21_ns, features = "nUMI", pt.size = 0.1) + ggtitle("UMI Distribution (Violin Plot)")
hist_plot <- ggplot(data = data.frame(nUMI = pb21_ns$nUMI), aes(x = nUMI)) +
...
results <- lapply(scales, function(sf) process_and_score_obj(pb21_ns, scale_factor = sf))
...
scales <- c(3000, 5000, 10000)
results <- lapply(scales, function(sf) process_score_silhouette(pb21_ns, scale_factor = sf))
...
pb21_ns <- NormalizeData(pb21_ns, normalization.method = "LogNormalize", scale.factor = 10000)

pb21_ns <- FindVariableFeatures(pb21_ns, selection.method = "vst", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(pb21_ns), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(pb21_ns)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1 + plot2

pb21_ns_mvp <- FindVariableFeatures(pb21_ns, selection.method = "mvp", nfeatures = 2000)

# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(pb21_ns_mvp), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(pb21_ns_mvp)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1 + plot2

all.genes <- rownames(pb21_ns)
pb21_ns <- ScaleData(pb21_ns, features = all.genes)

pb21_ns <- RunPCA(pb21_ns, features = VariableFeatures(object = pb21_ns))

# Examine and visualize PCA results a few different ways
print(pb21_ns[["pca"]], dims = 1:5, nfeatures = 5)

DimPlot(pb21_ns, reduction = "pca") + NoLegend()

ElbowPlot(pb21_ns)

pb21_ns <- FindNeighbors(pb21_ns, dims = 1:10)
...
pb21_ns <- FindNeighbors(pb21_ns, dims = 1:10)  # Run once before clustering

for (res in resolutions) {
  pb21_ns <- FindClusters(pb21_ns, resolution = res, verbose = FALSE)
  colname <- paste0("RNA_snn_res.", res)
  newname <- paste0("clusters_res_", gsub("\\.", "_", res))
  pb21_ns[[newname]] <- pb21_ns[[colname]]
}

sapply(paste0("clusters_res_", gsub("\\.", "_", resolutions)), function(x) {
  length(unique(pb21_ns[[x]][,1]))
})

library(patchwork)

plots <- lapply(resolutions, function(res) {
  DimPlot(pb21_ns, group.by = paste0("clusters_res_", gsub("\\.", "_", res)),
...
pb21_ns <- FindNeighbors(pb21_ns, dims = 1:10, verbose = FALSE)
...
for (res in resolutions) {
  ...
  # Run clustering
  obj <- FindClusters(pb21_ns, resolution = res, verbose = FALSE)
  colname <- paste0("RNA_snn_res.", res)
...
pb21_ns <- FindClusters(pb21_ns, resolution = 0.6)

pb21_ns <- RunUMAP(pb21_ns, dims = 1:10)

DimPlot(pb21_ns, reduction = "umap")

# find markers for every cluster compared to all remaining cells, report only the positive
# ones
pb21_ns.markers <- FindAllMarkers(pb21_ns)

pb21_ns.markers %>%
  group_by(cluster) %>%
  dplyr::filter(avg_log2FC > 1)

cluster_tables <- pb21_ns.markers %>%
  group_by(cluster) %>%
  group_split()
...
VlnPlot(pb21_ns, features = c("MS4A1", "CD79A"), slot = "counts", log = TRUE)
...
library(viridis)
DoHeatmap(pb21_ns, features = top10$gene) +
